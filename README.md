...
*검증 헤더와 조건부 요청
정리
-캐시 시간이 유효 시간이 초과해도, 서버의 데이터가 갱신되지 않으면
...
*검증 헤더와 조건부 요청
Last-Modified, If-Modified-Sinc 단점
...
*프록시 캐시 도입
-웹 브라우저1 )
-웹 브라우저2 ) ----100ms (0.1초)---- 프록시 캐시 서버 (한국 어딘가..) ----400ms (0.5초)---- 미국에 있는 원 서버
-웹 브라우저3 )
...
-Cache-Control: private
 -응담이 해당 사용자만을 위한 것임, private 캐시에 저장해야함(기본값)
...
no-cache 기본 동작
-웹 브라우저 -> 1. 캐시 서버 요청 no-cache + ETag -> 프록시 캐시 -> 2. 원서버 요청 no-cache + ETag -> 원서버 - 3.원서버 검증 -> 4. 응답 304 Not Modified -> 프록시 캐시 -> 5. 응답 304 Not Modified -> 웹 브라우저 - 6. 캐시 데이터 사용

#9. 다음으로(학습 내용 정리)
*실무에 꼭 필요한 HTTP 핵심 학습 완료

*HTTP에 대해서 더 깊이있게 학습
1. HTTP 스펙
 -RFC2616: https/://tools.ietf.org/html/rfc2616 -> 주의! 이걸로 보면 안됨
 -RFC7230~7235: https/://tools.ietf.org/html/rfc7230 -> 이걸로 모두 개정
...
*'스프링 핵심 원리 -기본편'과 'HTTP 기본지식'을 학습하고 나면 본격적으로 스프링 기반의 웹 기술인 '스프링 웹 MVC'를 제대로 학습할 준비가 되었다.

*추천 코스
1.스프링 입문
2.스프링 핵심 원리
3.모든 개발자를 위한 HTTP 웹 기본지식
